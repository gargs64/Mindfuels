...
const req = https.request({
  ...
  headers: {
    ...
    'Authorization': 'Token 2775fa3991bf14c78836dedaf6a1b3a1ff500716b71ffd79278f9d3219821bc3'
  }
}, res => {
  let chunks = [];
  res.on('data', d => chunks.push(d));
  res.on('end', () => console.log('Token:', Buffer.concat(chunks).toString()));
});
req.write(data);
req.end();
...
const req2 = https.request({
  ...
  let chunks = [];
  res.on('data', d => chunks.push(d));
  res.on('end', () => console.log('Bearer:', Buffer.concat(chunks).toString()));
});
req2.write(data);
req2.end();
